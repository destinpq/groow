...
const SecurityEnhancementsPage: React.FC = () => {
  ...
  const securityScore = logStats?.totalLogs ? Math.min(100, 90 + Math.floor(logStats.totalLogs / 1000)) : 92;
  const threatsBlocked = logStats?.securityLogs || 142;
  const activeAlerts = securityLogs.filter((e) => e.status !== 'resolved').length;
